...
import com.oakinvest.b2g.bitcoin.dto.bitcoin.core.util.BitcoinCoreResponseError;
...
/**
 * Bitcoin core mock for tests.
 * Created by straumat on 04/01/17.
 */
@Configuration
@Aspect
@Profile("test")
public class BitcoinCoreMock {
     ...
     * core directory.
     */
    private static final String BITCOIN_CORE_CACHE_DIRECTORY = "src/test/resources/cache";

    /**
     * getblock directory.
     */
    private static final String GET_BLOCK_CACHE_DIRECTORY = BITCOIN_CORE_CACHE_DIRECTORY + "/getblock";

    /**
     * getblockcount directory.
     */
    private static final String GET_BLOCK_COUNT_CACHE_DIRECTORY = BITCOIN_CORE_CACHE_DIRECTORY + "/getblockcount";

    /**
     * getBlockHash directory.
     */
    private static final String GET_BLOCK_HASH_CACHE_DIRECTORY = BITCOIN_CORE_CACHE_DIRECTORY + "/getBlockHash";

    /**
     * getRawTransaction directory.
     */
    private static final String GET_RAW_TRANSACTION_CACHE_DIRECTORY = BITCOIN_CORE_CACHE_DIRECTORY + "/getRawTransaction";

    /**
     * Logger.
     */
    private final Logger log = LoggerFactory.getLogger(BitcoinCoreMock.class);
     ...
     * Default constructor.
     */
    public BitcoinCoreMock() {
        final File bitcoinCoreDirectory = new File(BITCOIN_CORE_CACHE_DIRECTORY);
        if (!bitcoinCoreDirectory.exists() && !bitcoinCoreDirectory.mkdir()) {
            log.error("Impossible to create " + bitcoinCoreDirectory.getAbsolutePath());
        }
     ...
     */
    @Around("execution(* com.oakinvest.b2g.bitcoin.service.BitcoinCoreService.getBlockCount())")
    public final Object getBlockCount(final ProceedingJoinPoint pjp) throws Throwable {
        ...
        // if the file doesn't exists, we call the core server and save the file.
        if (!response.exists()) {
        ...
        if (randomNumber == 1) {
            // We create an error.
            BitcoinCoreResponseError error = new BitcoinCoreResponseError();
            error.setCode(0);
     ...
     * @throws Throwable exception.
     */
    @Around("execution(* com.oakinvest.b2g.bitcoin.service.BitcoinCoreService.getBlockHash(..)) && args(blockHeight)")
     ...
     * @throws Throwable exception.
     */
    @Around("execution(* com.oakinvest.b2g.bitcoin.service.BitcoinCoreService.getBlock(..)) && args(blockHash)")
     ...
     * @throws Throwable exception.
     */
    @Around("execution(* com.oakinvest.b2g.bitcoin.service.BitcoinCoreService.getRawTransaction(..)) && args(transactionHash)")
