...
public class BitcoinCoreServiceTest extends BaseTest {
	 ...
	 */
	private static final long MAXIMUM_RESPONSE_TIME_IN_MINUTES = 5;
	...
	public final void getBlockCountTest() {
		assertThat(getBitcoinCoreService().getBlockCount().getError())
				.as("Check that the api does not return an error")
				.isNull();

		assertThat(getBitcoinCoreService().getBlockCount().getResult())
	...
	public final void getBlockCountTestInError() {
		with().await().atMost(MAXIMUM_RESPONSE_TIME_IN_MINUTES, MINUTES)
				.until(() -> getBitcoinCoreService().getBlockCount().getError() != null);
	}
	...
	public final void getBlockHashTest() {
		assertThat(getBitcoinCoreService().getBlockHash(BLOCK_NUMBER_VALID).getError())
				.as("Check that the api does not return an error")
				.isNull();

		assertThat(getBitcoinCoreService().getBlockHash(BLOCK_NUMBER_VALID).getResult())
	...
	public final void getBlockHashTestInError() {
		assertThat(getBitcoinCoreService().getBlockHash(BLOCK_IN_ERROR_1).getError())
		...
		for (int i = 0; i < NUMBER_OF_ERRORS; i++) {
			getBitcoinCoreService().getBlockHash(BLOCK_IN_ERROR_1);
		}
	...
	public final void getBlockTest() {
		...
		// Test.
		GetBlockResult blockResult = getBitcoinCoreService().getBlock(BLOCK_HASH).getResult();
	...
	public final void getBlockTestInError() {
		assertThat(getBitcoinCoreService().getBlock(BLOCK_HASH_IN_ERROR_1).getError())
				.as("Check that the api does return an error")
				.isNotNull();

		for (int i = 0; i < NUMBER_OF_ERRORS + 1; i++) {
			getBitcoinCoreService().getBlock(BLOCK_HASH_IN_ERROR_1);
		}
	...
	public final void getRawTransactionTest() {
		...
		// Test.
		// Transaction.
		GetRawTransactionResult transactionResult = getBitcoinCoreService().getRawTransaction(BLOCK_EXISTING_TRANSACTION_HASH).getResult();
		...
		GetRawTransactionResult coinbaseTransaction = getBitcoinCoreService().getRawTransaction(COINBASE_TRANSACTION_HASH).getResult();
	...
	public final void getRawTransactionInError() {
		assertThat(getBitcoinCoreService().getRawTransaction(TRANSACTION_HASH_IN_ERROR_1).getError())
				.as("Check that the api does return an error")
				.isNotNull();

		// After NUMBER_OF_ERRORS errors, the service should return to normal.
		for (int i = 0; i < NUMBER_OF_ERRORS; i++) {
			getBitcoinCoreService().getRawTransaction(TRANSACTION_HASH_IN_ERROR_1);
		}
	...
	public final void getRawTransactionWithEmptyAddressesTest() {
		...
		assertThat(getBitcoinCoreService().getRawTransaction(invalidTransaction).getResult().getVout().get(1).getScriptPubKey().getAddresses())
				.as("Empty address")
				.isEmpty();
	}
	...
	public final void errorManagementTest() {
		...
		// Test.
		assertThat(getBitcoinCoreService().getBlockHash(nonExistingBlockNumber).getError())
				.as("Error")
				.isNotNull()
